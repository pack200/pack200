 ...
 */
public class BandIntegrityTest {

    @Test
    public void testBand() throws IOException {
        File testFile = new File("target/test.jar");
        TestUtils.jar("cvf", testFile.getAbsolutePath(),
                "-C", TestUtils.TEST_CLS_DIR.getAbsolutePath(),
                ".");
        ...
        File configFile = new File("target/pack.conf");
        TestUtils.createFile(configFile, scratch);
        File outFile = new File("target/out.jar");
        TestUtils.repack(testFile, outFile, true,
                "-v", "--config-file=" + configFile.getAbsolutePath());
