...
public class AttributeTest {
    ...
    public void testMethodParameters() throws Exception {
        ...
        String javaClassName = fname + TestUtils.CLASS_FILE_EXT;
        ...
        TestUtils.compiler(javaFile.getAbsolutePath(), "-parameters");

        // jar the file up
        File testjarFile = new File(cwd, "test" + TestUtils.JAR_FILE_EXT);
        TestUtils.jar("cvf", testjarFile.getAbsolutePath(), "-C", "target", javaClassName);

        TestUtils.testWithRepack(testjarFile, "--unknown-attribute=error");
    }
     ...
     */
    @Test
    public void test6746111() throws Exception {
        File badAttrJar = new File("target/test-classes", "badattr.jar");
        //TestUtils.copyFile(new File(TestUtils.TEST_SRC_DIR, "badattr.jar"), badAttrJar);
        File testJar = new File("target", "test.jar");
        List<String> cmds = new ArrayList<String>();
        cmds.add(TestUtils.getJavaCmd());
        cmds.add("-Duser.language=en");
        cmds.add("-cp");
        cmds.add("target/classes");
        cmds.add("io.pack200.Driver");
        cmds.add("--repack");
        cmds.add("-v");
        cmds.add(testJar.getAbsolutePath());
        cmds.add(badAttrJar.getAbsolutePath());
        List<String> output = TestUtils.runExec(cmds);
        /*
         * compare the repacked jar bit-wise, as all the files
         * should be transmitted "as-is".
         */
        TestUtils.doCompareBitWise(badAttrJar.getAbsoluteFile(), testJar.getAbsoluteFile());
    ...
    private void findString(String outputStr, List<String> notfoundList,
            String[] expectedStrings) {
