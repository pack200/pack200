...
public class CommandLineTest {
    ...
    private static final File ConfigFile = new File("target", "pack.conf");
    private static final List<File> jarList;

    static {
        javaCmd = TestUtils.IsWindows
                    ? new File(EXP_SDK_BIN_DIR, "java.exe")
                    : new File(EXP_SDK_BIN_DIR, "java");

        javacCmd = TestUtils.IsWindows
    ...
    static void init() throws IOException {
        TestUtils.recursiveCopy(TestUtils.JavaSDK, EXP_SDK);
            creatConfigFile();
    }
    // cleanup the test area
    static void cleanup() throws IOException {
        TestUtils.recursiveDelete(EXP_SDK);
        TestUtils.cleanup();
    }
    ...
    static void runPack200(boolean jre) throws IOException {
        ...
        for (File f : jarList) {
            ...
            File bakFile = new File("target", f.getName() + ".bak");
            if (!bakFile.exists()) {  // backup
                TestUtils.copyFile(f, bakFile);
            } else {  // restore
                TestUtils.copyFile(bakFile, f);
            }
            cmdsList.clear();
            ...
            cmdsList.add(TestUtils.Is64Bit ? "-Xmx1g" : "-Xmx512m");
